...
async function registerMonacoThemes(themes: string[], languages: string[]) {
...
export function useMonaco(monacoOptions: MonacoOptions = {}) {
  ...
  let initialTheme: MonacoTheme = isDark.value ? themes[0] : themes[1]
  const currentTheme = computed<MonacoTheme>(() => isDark.value ? themes[0] : themes[1])
